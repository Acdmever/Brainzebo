...
void initializeEepromVariables(){
  ...
  setVpVersion(VP_VERSION);
  writeEepromSettings();
  safeWriteEeprom(999,0);
  //light LED to indicate initialization complete
  processLed(50,50,50);
  noDelay(1000);
  processLed(0,0,0);
}
...
/****************************************************************************************************************
*READ and SET supergroup and subgroup
****************************************************************************************************************/
...
void setFirmwareFlashFlag(byte val)
{
  newDataFlag(); 
  firmwareFlashFlag=val;
  setLED(deviceID);
}
...
void displaySupergroup(){
  if(TACTILE_VP){
    ...
    byte fives = (supergroup-50) / 5;
    byte ones = (supergroup-50) % 5;
    for(byte i=0;i<fives;i++){
      processLed(0,0,50);
      noDelay(500);
      processLed(0,0,0);
...
void displaySubgroup(){
  for(byte i=120;i<subgroup;i++){
    processLed(25,25,0);
    noDelay(100);
    processLed(0,0,0);
    noDelay(400);
...
void displayVpVersion(){
  for(byte i=0;i<VP_VERSION;i++){
    processLed(25,25,25);
    noDelay(200);
    processLed(0,0,0);
    noDelay(200);
...
int getSystemParameters(byte num){
  switch(num){
    ...
    case 3: return firmwareFlashFlag;
    case 4: return VP_VERSION;
  }
  return 256;
}
...
int setSystemParameters(byte num, byte val){
  switch(num){
    ...
    case 2: subgroup=val; return 0; break;
    case 3: firmwareFlashFlag=val; return 0; break;
    case 4: VP_VERSION=val; return 0; break;
  }
  return 256;
}
